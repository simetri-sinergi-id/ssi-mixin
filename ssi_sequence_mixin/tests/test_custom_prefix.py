...
class TestMixinSequence(BaseCase):
    @file_data("scenario_custom_prefix.yml")
    def test_error_sequence_custom_prefix(self, sequence, template, test_model):
        # Test sequence dengan custom prefix
        ...
        template_prefix = self.env["sequence.template"].create(
            {
                "name": template["name"],
        ...
        self.assertEqual(template, template_prefix)
        # Raise Error
        with self.assertRaises(UserError):
            rec._create_sequence()

    @file_data("scenario_custom_prefix.yml")
    def test_sequence_custom_prefix(self, sequence, template, test_model):
        # Test sequence dengan custom prefix
        sequence_prefix = self.env["ir.sequence"].create(
            {
                "name": sequence["name"],
        ...
        self.assertEqual(template, template_prefix)
        rec._create_sequence()
        # Pastikan name dimulai dengan prefix
        self.assertEqual(rec.name, "TEST-PREFIX/000001")
